define(["lib-build/css!./MainView",
    ...
    "storymaps/common/mapcontrols/layerlist/LayerList",
    ...
    function (
        ...
        return function MainView(builderView) {
            ...
            this.init = function (core) {
                ...
                */
                ...
                $(document).bind(
                    ...
                    function (e) {
                        //console.log($(e.target).parents().each(function(i, p){ console.log(p.className); }));
                        if (!$(e.target).parents('.layerListContainer, #descLegendPanel, #accordionPanel, .mobilePopup, #headerMobile, .settings-layout').length)
                            e.preventDefault();
                    }
                );
            ...
            this.getMapConfig = function (response, mapContainer) {
                return {
                    ...
                    layerlist: new LayerList(
                        response,
                        app.isInBuilder,
                        getLayerListSettings(mapContainer, response.itemInfo.item.id)
                    ),
            ...
            function getLayerListSettings(mapContainer, webmapId) {
                var placement = WebApplicationData.getLayerListPlacement(),
                    container = mapContainer.siblings('.layerList');

                if ($("body").hasClass("mobile-view")) {
                    placement = "panel";
                    container = mapContainer.siblings('.mobilePopup').find('.layerListMobile');
                }
                else if (placement == "panel")
                    container = app.ui.descLayerListPanel.getLayerListContainer(webmapId);
            ...
            this.updateUI = function () {
                ...
                $.each(Object.keys(app.maps), function (i, id) {
                    ...
                    app.maps[id].layerList.updatePlacementSettings(
                        getLayerListSettings($(app.maps[id].response.map.container), id)
                    );
                });
                        ...
                        $.each(sizesOptions, function (i, size) {
                            ...
                            */
                            ...
                            CommonHelper.addCSSRule(
                                ...
                                //LayerList
                            CommonHelper.addCSSRule(
                                bodyPrefix + layout.id + "-right-" + size + " .mainMediaContainer .layerListContainer"
                                + "{ right:" + targetSize + "; }");
